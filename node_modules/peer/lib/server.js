...
app._initializeHTTP = function() {
  ...
  this.get('/peers', function(req, res, next) {
    var key = req.params.key;
    if (self._clients) {

      self._log('---------------- trying to get the data ----------------');
      self._log(self._clients)
      self._log('---------------- trying to get the data ----------------');
...
app._startStreaming = function(res, key, id, token, open) {
  ...
  if (open) {
    var payload = {
      clientIds: Object.keys(this._clients['peerjs'])
    };
    res.write(JSON.stringify({ type: 'OPEN', payload: payload }) + '\n');
  }
